...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Calculate quaternion difference to determine orientation error
    orientation_diff = quat_mul(object_rot, quat_conjugate(goal_rot))
    orientation_distance = 2.0 * torch.acos(torch.clamp(orientation_diff[:, 3], -1.0, 1.0))
    
    # Reward component for aligning the object's orientation with the goal
    orientation_alignment_reward = -orientation_distance

    # Reward component for promoting rotation
    rotation_reward_scale = 0.1
    rotation_reward = rotation_reward_scale * torch.norm(object_angvel, dim=1)
    
    # Apply transformations to the rewards
    orientation_temp = 10.0
    transformed_orientation_reward = torch.exp(orientation_temp * orientation_alignment_reward)
    
    rotation_temp = 1.0
    transformed_rotation_reward = torch.exp(rotation_temp * rotation_reward)
    
    # Compute total reward
    total_reward = transformed_orientation_reward + transformed_rotation_reward
    
    # Construct the reward components dictionary
    reward_dict = {
        'orientation_alignment_reward': transformed_orientation_reward,
        'rotation_reward': transformed_rotation_reward
    }
